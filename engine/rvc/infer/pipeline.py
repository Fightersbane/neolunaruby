"""Vendored from ultimate_rvc.rvc.infer.pipeline, trimmed for neolunamiku.

Removed relative to upstream: CREPE and FCPE f0 methods (torchcrepe/torchfcpe
dependencies); only rmvpe is supported. NOTE: engine.pipeline patches this
module's RMVPE and faiss.read_index attributes with functools.cache, so keep
both names importable at module level.
"""
...
from engine.rvc.lib.predictors.f0 import RMVPE
...
class Pipeline:
    ...
    def get_f0(
        ...
        if f0_method == "rmvpe":
            ...
            msg = (
                f"f0 method '{f0_method}' was removed from the slim build; "
                "only 'rmvpe' is supported"
            )
